...
export class BotService implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit(): void {
    ...
    const clientOptions = proxyUrl
      ...
      : {};
    if (proxyUrl) this.downloadDispatcher = createUndiciProxyDispatcher(proxyUrl);
    ...
    void this.bot
      .start({ onStart: (me) => this.logger.log(`Telegram-бот запущен: @${me.username}`) })
      .catch((err) => this.logger.error(`Не удалось запустить бота: ${(err as Error).message}`));
  }
  ...
  private async handleDocument(ctx: Context): Promise<void> {
    ...
    if (!interval) {
      await ctx.reply('Не нашёл интервал в подписи. Укажите два числа, например «590-625» или «590 625».');
      return;
    }
    ...
    try {
      ...
      await ctx.replyWithPhoto(new InputFile(artifacts.heatmapPng, 'heatmap.png'), {
  ...
  private async downloadDocument(ctx: Context, fileId: string): Promise<Buffer> {
